...
async function addRole() {
  ...
  prompt([
    ...
    pool
      .query(
        "INSERT INTO role (title, salary, department) VALUES ($1, $2, $1)",
...
async function updateEmployeeRole() {
  ...
  prompt([
    ...
    const role_id = roles.filter((obj) => obj.title === role)[0].id;
    const employee_id = employee.filter((obj) => obj.first_name + " " + obj.last_name === employee)[0].id;
    pool
  ...
  .catch((err) => {
    console.log("Error when updating employee role!");
    console.log(err);
  });
}
